...
def _get_image_blob(im):
    ...
    # MP: alternative to below. This can be done because this only affects the inference phase.
    ...
    # MP: end of alternative   

    """
    for target_size in cfg.TEST.SCALES: #cfg.TEST.SCALES = 600
	im_scale = float(target_size) / float(im_size_min)
	# MP: modified, but does the same
        im_scale_max = float(cfg.TEST.MAX_SIZE) / float(im_size_max)
	im_scale = min(im_scale, im_scale_max)
...
def im_detect2(net, im, boxes=None):
    ...
    """"
    # MP: remove below if statement because it isn't used
    if cfg.DEDUP_BOXES > 0 and not cfg.TEST.HAS_RPN:
        v = np.array([1, 1e3, 1e6, 1e9, 1e12])
        hashes = np.round(blobs['rois'] * cfg.DEDUP_BOXES).dot(v)
        _, index, inv_index = np.unique(hashes, return_index=True,
    ...
    # MP: below if statement replaced by following if statement
    if cfg.TEST.HAS_RPN:
    ...
    if cfg.TEST.HAS_RPN:
        ...
        blobs['im_info'] = np.array(
            ...
            dtype=np.float32)
    """
    # MP: Prepare network for data injection
    # reshape network inputs
    net.blobs['data'].reshape(*(blobs['data'].shape))
    if cfg.TEST.HAS_RPN:
        net.blobs['im_info'].reshape(*(blobs['im_info'].shape))
    ...
    # MP: group blobs in forward_kwargs to inject it into network
    # do forward
    forward_kwargs = {'data': blobs['data'].astype(np.float32, copy=False)}
    if cfg.TEST.HAS_RPN:
        forward_kwargs['im_info'] = blobs['im_info'].astype(np.float32, copy=False)
    ...
    masks_out = blobs_out['mask_prob'] # shape of masks_out blob: (n, 10, 244, 244), where n is the number of bboxes and 10 is the number of mask classes.


    #scores = net.blobs['cls_score'].data.copy()
    #bbox_deltas = net.blobs['bbox_pred'].data.copy()
    #pred_boxes = bbox_transform_inv(boxes, bbox_deltas)
    #pred_boxes = clip_boxes(pred_boxes, im.shape)

    # MP:  NOTE: rois_final is basically the same to pred_boxes, seems overwork to compute pred_boxes. The same holds for scores, this information is already stored under rois_cls_score. Furthermore, it isn't used anywhere in the `run_affordance_net(net, image_names) method of demo_img.py. Therefore these two are ommited in the return statemen and above computations are commented out.

    #return rois_final, rois_class_score, rois_class_ind, masks_out, scores, pred_boxes
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print("masks_out shape: {}".format(masks_out.shape))
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    return rois_final, rois_class_score, rois_class_ind, masks_out
    ...
    """
    """
    # MP: commented out because not used
    if cfg.DEDUP_BOXES > 0 and not cfg.TEST.HAS_RPN:
        # Map scores and predictions back to the original set of boxes
        scores = scores[inv_index, :]
        pred_boxes = pred_boxes[inv_index, :]
